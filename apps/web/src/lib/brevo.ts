...
export function addTracking(html: string, campaignId: string, recipientId: string, unsubscribeUrl: string) {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "https://pandacrm.com.ng";
  const openUrl = `${baseUrl}/api/campaigns/track/open?recipient=${recipientId}`;
  const trackedHtml = html.replace(/href\s*=\s*("|')([^"']+)\1/gi, (match, quote, url: string) => {
    if (url === unsubscribeUrl) return match;
    if (!url.startsWith("http")) return match;
    const clickUrl = `${baseUrl}/api/campaigns/track/click?recipient=${recipientId}&url=${encodeURIComponent(url)}`;
